...
import utilities.training_stats as training_stats
...
class EDM():
    ...
    def sample_ptrain_safe(self,N):
        ...
        """
        a=torch.rand(N)
        t=(self.sigma_max**(1/self.ro) +a *(self.sigma_min**(1/self.ro) - self.sigma_max**(1/self.ro)))**self.ro
        return t
    ...
    def loss_fn(self, net, x):
        ...
        #input_ov, _, cnoise= self.prepare_train_preconditioning(x_ov, sigma)

        print("inputs to net", input.shape, cnoise.shape)
        #print("inputs to net", input_ov.shape, cnoise.shape)
        #in_net=torch.cat((input, input_ov), dim=1)
...
#----------------------------------------------------------------------------
class Trainer():
    def __init__(self, args, dset, network, optimizer, diff_params,  device='cpu'):
        self.args=args
        self.dset=dset
        ...
        if self.args.train.resume:
            if self.args.train.resume_checkpoint != "None":
                resuming =self.resume_from_checkpoint(checkpoint_path=self.args.train.resume_checkpoint)
            else:
    ...
    def get_batch(self):
            ...
            #self.resample(audio)
            audio=torchaudio.functional.resample(audio, self.args.dset.fs, self.args.exp.sample_rate)
            #print(audio.shape)
            #print("before asser", audio.shape)
            print(audio.shape, self.args.exp.seg_len)
            #assert audio.shape[1] == self.args.exp.audio_len
            #print statistics of the samples
            #print("mean", audio.mean().item(), "std", audio.std().item())
    ...
    def train_step(self):
        ...
        if self.it <= self.args.train.lr_rampup_it:
            for g in self.optimizer.param_groups:
                ...
                g['lr'] = self.args.train.lr * min(self.it / max(self.args.train.lr_rampup_it, 1e-8), 1)
        ...
        if self.args.train.use_grad_clip:
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), self.args.train.max_grad_norm)
...
def _main(args, mode):
    ...
    optimizer = torch.optim.Adam(network.parameters(), lr=args.train.lr, betas=(args.train.optimizer.beta1, args.train.optimizer.beta2), eps=args.train.optimizer.eps)

    dset=iter(torch.utils.data.DataLoader(dataset=dataset_obj, batch_size=args.train.batch,  num_workers=args.dset.num_workers, pin_memory=True))
...
def main():
    ...
    parser.add_argument('--MODE', type=str, default=None) #"noise", "trajectories" or "toytrajectories"
